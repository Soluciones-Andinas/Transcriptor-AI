...
|---|---|---|---|---|
| API HTTP | FastAPI 0.115 + Uvicorn 0.32 | Estándar Python; async; OpenAPI; compatible con MCP SDK (Simplicidad) | Versión Python con CUDA wheels específica | Pinear Python 3.10 en Dockerfile (D-056: `pyproject.toml` permite `>=3.10,<3.12` para dev local, pero el container productivo fija 3.10 con `ln -sf /usr/bin/python3.10 /usr/bin/python`; alinea CUDA 12.1 wheels con el runtime). |
| MCP Server | `mcp[server]` 1.5.x (SDK Anthropic) + bearer middleware custom | Streamable HTTP transport; tools/resources nativos (Simplicidad) | SDK joven, breaking changes posibles | Pin de versión `>=1.5,<2.0` en `pyproject.toml` (D-057); tests de contrato + audit antes de subir minor. |
| Runtime readiness gate | `src/transcription_api/runtime/readiness.py` | Centraliza AC-15 (`MODELS_NOT_LOADED` 503) para REST + MCP — evita duplicación de la precedencia `whisper_status` vs `pyannote_status` (D-055). | — | Llamado desde `api/transcriptions.py` + `mcp/tools/*` antes de invocar el pipeline. |
